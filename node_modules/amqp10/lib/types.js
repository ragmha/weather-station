...
function arrayBuilder(val, bufb, codec, width) {
  ...
  if (!types.hasOwnProperty(val.elementType)) {
    throw new errors.EncodingError('Unable to encode AMQP Array for type: ' + val.elementType + '.  Type encoder not found.');
  }

  var encoder = types[val.elementType];
  var _len = val.array.length;
  for (var _i = 0; _i < _len; ++_i) encoder.encode(val.array[_i], tempBufb, codec);
...
registerType('symbol', {
  encoder: function(val, bufb) {
    var encoded = (val instanceof AMQPSymbol) ?
      new Buffer(val.toString(), 'ascii') : new Buffer(val, 'ascii');
  ...
  encodings: [
    encoding(0xa3, {
      ...
      decoder: function(buf) { return new AMQPSymbol(buf.slice(1).toString('ascii')); }
    }),
    encoding(0xb3, {
      encoder: function(val, bufb) {
        var encoded = (val instanceof Buffer) ? val : new Buffer(val, 'ascii');
      ...
      decoder: function(buf) { return new AMQPSymbol(buf.slice(4).toString('ascii')); }
    })
  ]
});
